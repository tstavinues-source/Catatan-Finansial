...
// Tambahan Import Firebase & Logger sesuai saran Anda
...
window.reCalculateAll = function() {
    ...
    AuraUtils.safeDOM('stats-proj-mth', el => el.innerText = AuraUtils.formatCurrency(proj));
    ...
    // Staples Trackers
    for (let i = 0; i < trackerIds.length; i++) {
        AuraUtils.safeDOM(`track-${trackerIds[i]}`, el => el.innerText = AuraUtils.formatCurrency(trackerBalances[trackerIds[i]]));
    }

    // Category Distribution
    const catSorted = Object.entries(catSpend).sort((a, b) => b[1] - a[1]);
    AuraUtils.safeDOM('pie-total-label', el => el.innerText = AuraUtils.formatCurrency(periodSpent));
        ...
        if (catSorted.length === 0) { 
            ...
            for (let i = 0; i < catSorted.length; i++) {
                ...
                <div class="flex justify-between items-center text-sm border-b border-[var(--border-glass)] pb-2.5 last:border-0 last:pb-0">
                    ...
                    <p class="font-mono text-xs font-bold text-[var(--text-main)]">${AuraUtils.formatCurrency(catSorted[i][1])}</p>
                </div>`;
            }
        ...
        if (merchSorted.length === 0) { 
            ...
            for (let i = 0; i < merchSorted.length; i++) { 
                ...
                <div class="flex justify-between items-center text-sm border-b border-[var(--border-glass)] pb-2.5 last:border-0 last:pb-0">
                    ...
                    <span class="font-mono font-bold text-[var(--color-expense)]">${AuraUtils.formatCurrency(merchSorted[i][1])}</span>
                 </div>`; 
            }
            ...
            for (let j = 0; j < g.items.length; j++) {
                ...
                if (hasItems) {
                    ...
                    for (let k = 0; k < t.items.length; k++) {
                        ...
                        <div class="flex justify-between items-center text-xs bg-white/5 p-2 rounded-xl group/it">
                            ...
                            <span class="font-mono text-[var(--text-muted)] text-[11px] mr-2">${AuraUtils.formatCurrency(AuraUtils.convertCurrency(it.harga * (it.qty || 1), t.mata_uang))}</span>
                ...
                <div class="glass-panel p-4 relative group">
                    ...
                    <div class="flex justify-between items-start mb-2 pr-12">
                        ...
                        <p class="font-bold text-sm font-mono shrink-0 ml-2 ${colorClass}">${signChar}${AuraUtils.formatCurrency(AuraUtils.convertCurrency(t.nominal, t.mata_uang))}</p>
                    </div>
