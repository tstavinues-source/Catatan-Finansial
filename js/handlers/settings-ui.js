...
window.saveUserProfile = async function() {
    ...
    if (!name || !nick) {
        if (window.showToast) {
            window.showToast("Nama dan Panggilan tidak boleh kosong!", true);
        }
        return;
    }
    ...
    try {
        ...
        if (window.showToast) {
            window.showToast("Profil, Negara & Mata Uang berhasil disimpan!");
        }
    } catch (e) {
        if (window.showToast) {
            window.showToast("Gagal menyimpan profil.", true);
        }
    }
};
...
window.saveAIPreferences = async function() {
    ...
    try {
        ...
        if (window.showToast) {
            window.showToast("Gagal menyimpan preferensi AI.", true);
        }
    }
};
...
window.addGroqKey = async function() {
    ...
    if (!key || !key.startsWith('gsk_')) {
        if (window.showToast) {
            window.showToast("Format API Key tidak valid!", true);
        }
        return;
    }
    ...
    try {
        ...
        if (window.showToast) {
            window.showToast(`Kunci Groq Ke-${currentKeys.length} diamankan ke Cloud!`);
        }
        window.renderGroqKeysUI();
    } catch(e) {
        if (window.showToast) {
            window.showToast("Gagal mengunggah kunci ke Cloud.", true);
        }
    }
};
...
window.renderGroqKeysUI = function() {
    AuraUtils.safeDOM('groq-keys-container', function(el) {
        ...
        if (keys.length === 0) {
            ...
            if(badge) {
                badge.className = "text-[9px] bg-red-950/40 text-rose-400 border border-red-900/50 px-2 py-0.5 rounded font-mono";
        ...
        if(badge) {
            badge.className = "text-[9px] bg-emerald-950/40 text-emerald-400 border border-emerald-900/50 px-2 py-0.5 rounded font-mono shadow-[0_0_10px_rgba(16,185,129,0.2)]";
        ...
        keys.forEach((encKey, index) => {
            ...
            <div class="flex justify-between items-center bg-[var(--bg-base)] p-2.5 rounded-xl border border-emerald-900/30 bg-emerald-900/10 mb-2 last:mb-0">
                <div class="flex flex-col">
                    <span class="font-mono text-xs font-bold ${statusColor} flex items-center gap-2">
                        <i class="fa-solid fa-cloud-check"></i> ${display}
                    </span>
                    <span class="text-[8px] text-[var(--text-muted)] uppercase tracking-wider mt-0.5">
                        Pool Ke-${index + 1} • Tersinkronisasi Firebase
                    </span>
                </div>
                <button onclick="window.removeGroqKey(${index})" class="text-rose-500 p-2 hover:text-rose-400 hover:bg-rose-500/10 rounded-lg active:scale-90 transition">
...
window.syncGeminiEngine = async function(silent = false) {
    ...
    if (!pin || pin.length < 3) {
        if (!silent && window.showToast) {
            window.showToast("HARAP MASUKKAN PIN GEMINI!", true);
        }
        return;
    }
    ...
    try {
        if(typeof window.GeminiFailoverEngine !== 'function') {
            throw new Error("Modul AI belum siap.");
        }
        ...
        if (gCount > 0) {
            ...
            if (!silent && window.showToast) {
                window.showToast("Gemini Vision Di-Unlock.");
            }
        } else {
            throw new Error("0 Keys");
        }
    } catch(e) {
        if (gBadge) {
            gBadge.className = "text-[9px] bg-red-950/40 text-rose-400 border border-red-900/50 px-2 py-0.5 rounded font-mono";
            gBadge.innerText = "LOCKED";
        }
        
        if (!silent && window.showToast) {
            window.showToast("Dekripsi Gagal: PIN Salah.", true);
        }
    }
};
...
window.addRecurringPayment = async function() {
    ...
    if(!name || isNaN(amt) || isNaN(date) || date < 1 || date > 31) {
        if(window.showToast) window.showToast("Form tidak valid!", true);
        return;
    }
    ...
    try {
        ...
        if(window.showToast) {
            window.showToast("Tagihan otomatis ditambahkan.");
        }
    } catch(e) {
        if(window.showToast) {
            window.showToast("Gagal menambah.", true);
        }
    }
};
...
window.renderRecurringUI = function() {
    AuraUtils.safeDOM('recurring-list', function(el) {
        ...
        if (entries.length === 0) {
            el.innerHTML = '<p class="text-[10px] text-[var(--text-muted)] text-center my-2">Mesin belum diajarkan mengenai rutinitas bulanan Anda.</p>';
            return;
        }
        ...
        entries.forEach(([id, rp]) => {
            ...
            <div class="glass-panel p-3 flex justify-between items-center border-l-2 border-l-sky-400 group">
                ...
                <p class="font-bold text-sm font-mono text-[var(--text-main)]">
                    ${AuraUtils.formatCurrency(rp.amount)}
                </p>
            </div>`;
        });
    ...
    Object.entries(trackers).forEach(([id, t]) => {
        ...
        <div class="glass-panel p-3 border-l-2 border-l-amber-400 flex justify-between items-center mb-2">
            <div>
                ...
                <p class="text-[9px] text-[var(--text-muted)] uppercase mt-0.5 leading-relaxed">
                    Keys: ${AuraUtils.escapeHtml(t.keywords.join(', '))}
                </p>
            </div>
            <button onclick="window.removeTracker('${id}')" class="text-rose-500 hover:text-rose-400 p-2">
...
window.saveNewTracker = async function() {
    ...
    if (!id || !name || keywords.length === 0) {
        if (window.showToast) {
            window.showToast("Isi ID, Nama, dan minimal 1 kata kunci!", true);
        }
        return;
    }
...
window.openFamilyManager = function() {
    ...
    members.forEach((m, i) => {
        ...
        <div class="glass-panel p-3 border-l-2 border-l-indigo-400 flex justify-between items-center mb-2">
            <p class="text-xs font-bold text-indigo-400">
                <i class="fa-solid fa-user mr-2"></i>${AuraUtils.escapeHtml(m)}
            </p>
            <button onclick="window.removeFamilyMember('${i}')" class="text-rose-500 hover:text-rose-400 p-2">
                <i class="fa-solid fa-trash"></i>
            </button>
        </div>`;
    });
    ...
    if (members.length === 0) {
        html = '<p class="text-[10px] text-[var(--text-muted)] text-center">Belum ada tanggungan keluarga.</p>';
    }
...
window.addFamilyMember = async function() {
    ...
    if (members.includes(name)) {
        if (window.showToast) {
            window.showToast("Anggota ini sudah terdaftar.", true);
        }
        return;
    }
    
    try {
        await FirebaseService.updateSettings({ familyMembers: [...members, name] });
        input.value = '';
        window.openFamilyManager();
    } catch (e) {
        if (window.showToast) {
            window.showToast("Gagal menambah anggota.", true);
        }
    }
};
...
window.openAuditLogs = async function() {
    ...
    <div class="text-center p-8">
        <i class="fa-solid fa-circle-notch animate-spin text-2xl text-white mb-2 block"></i>
        <p class="text-[10px] text-[var(--text-muted)]">Mengunduh blok rantai...</p>
    </div>`;
    ...
    try {
        ...
        if (snapshot.exists()) {
            const logsArray = Object.entries(snapshot.val())
                .map(([id, data]) => ({ id, ...data }))
                .sort((a, b) => b.ts - a.ts);
            ...
            logsArray.forEach(log => {
                ...
                <div class="border-b border-[var(--border-glass)] pb-2 mb-2 last:border-0 last:pb-0 last:mb-0">
                    <div class="flex justify-between items-start mb-1">
                        <span class="text-[9px] bg-white/10 text-white px-1.5 py-0.5 rounded font-mono">
                            ${AuraUtils.escapeHtml(log.action)}
                        </span>
                    ...
                    <p class="text-[8px] text-[var(--text-muted)] uppercase mt-1">
                        Executor: ${AuraUtils.escapeHtml(log.user)}
                    </p>
                </div>`;
            });
            ...
            container.innerHTML = '<p class="text-center text-xs text-[var(--text-muted)] p-5">Tidak ada riwayat aktivitas.</p>';
        }
    } catch(e) {
        container.innerHTML = '<p class="text-center text-xs text-rose-500 p-5">Gagal membaca log dari Cloud.</p>';
    }
};
